...
export async function sendCanonicalEmailCode(rawEmail:unknown){
  ...
  const resendWindowMs=60*60*1000,recentElapsed=recent.data?Date.now()-new Date(recent.data.last_sent_at).getTime():Infinity;
  if(recent.data){if(recentElapsed<60000)throw new PublicRequestCodeError('rate_limit_checked','SEND_COOLDOWN',429,'Please wait before requesting another code.');if(Number(recent.data.resend_count)>=5&&recentElapsed<resendWindowMs)throw new PublicRequestCodeError('rate_limit_checked','SEND_RATE_LIMITED',429,'Too many verification emails were requested. Please try again later.')}
  ...
  const resendCount=recent.data&&recentElapsed<resendWindowMs?Number(recent.data.resend_count)+1:0;
  const inserted=await database.from('public_request_verifications').insert({id,session_key_hash:sessionHash,normalized_email:email,purpose,code_digest:codeDigest(id,email,otp),expires_at:expires.toISOString(),last_sent_at:now.toISOString(),resend_count:resendCount}).select('id').single();
