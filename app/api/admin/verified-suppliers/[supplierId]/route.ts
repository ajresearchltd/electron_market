...
export async function PATCH(request:NextRequest,{params}:{params:Promise<{supplierId:string}>}){
  ...
  const current=await auth.admin.from('verified_supplier').select('supplier_id').eq('supplier_id',id).maybeSingle();if(!current.data)return fail('Verified supplier was not found.',404);
...
export async function PUT(request:NextRequest,{params}:{params:Promise<{supplierId:string}>}){
 ...
 const name=String(value('name')||'').trim(),delivery=String(value('delivery_product')||'').trim();let pic=String(value('pic')||'').trim()||null;const file=form?.get('file');
