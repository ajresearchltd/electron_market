...
import {requiresSupplierIdentificationConfirmation} from '../../../../../lib/rfqs/supplier-identification-confirmation';
...
export async function GET(_:Request,{params}:{params:Promise<{rfqId:string}>}){
  ...
  const [chain,progress,items,bom,preferences,messages,responses,allocations,invoices,workflow]=await Promise.all([
    ...
    auth.admin.from('procurement_invoices').select('id,invoice_number,invoice_status,procurement_chain_id,procurement_number,created_at').eq('source_rfq_id',rfqId).order('created_at',{ascending:false}),
  ...
  const allocationIds=(allocations.data??[]).map((row:any)=>row.id),approverIds=[...new Set((allocations.data??[]).map((row:any)=>row.selected_by).filter(Boolean))];
  const [bomItems,positions,runs,reviews,attachments,profiles,invoiceItems,approvers]=await Promise.all([
    ...
    allocationIds.length?auth.admin.from('procurement_invoice_items').select('invoice_id,source_allocation_id').in('source_allocation_id',allocationIds):Promise.resolve({data:[],error:null}),
    approverIds.length?auth.admin.from('user_profiles').select('id,role').in('id',approverIds):Promise.resolve({data:[],error:null}),
  ]);
  ...
  const rfqItemMap=new Map(rfqItems.map((row:any)=>[row.rfq_item_id,row]));
  ...
  const actorRoles=new Map((approvers.data??[]).map((row:any)=>[row.id,row.role])),canonical=buildCanonicalAllocationViews({allocations:allocations.data??[],invoiceItems:invoiceItems.data??[],invoices:invoices.data??[],rfqId,procurementChainId:chainId,customerId:header.data.customer_id,actorRoles});
  const buyerAllocations=(allocations.data??[]).map((row:any)=>{const view=canonical.get(row.supplier_response_item_id);return {...row,human_approved:Boolean(view),buyer_approved:Boolean(view),approved_by_role:view?.approvedByRole??null,approved_at:view?.approvedAt??null,included_in_invoice:Boolean(view?.isInvoiced),invoice_id:view?.invoiceId??null,invoice_number:view?.invoiceNumber??null}});
  return NextResponse.json({rfq:header.data,chain:chain.data,progress:progress.data,bom:bom.data,preferences:preferences.data,items:rfqItems,messages:supplierMessages,offers,unmatchedOffers:offers.filter((offer:any)=>!offer.rfq_item_id),reviews:reviews.data??[],allocations:buyerAllocations,invoice:invoices.data?.[0]??null,invoices:invoices.data??[],workflow:workflow.data??{execution_mode:'manual',automatic_rfq_to_invoice:false}},{headers:{'cache-control':'private, no-store'}});
}
