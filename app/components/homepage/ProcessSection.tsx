      ...
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, index) => {
            ...
            return (
              <article key={`${step.title}-${index}`} className="rounded-xl border border-slate-200 bg-white p-7 text-center shadow-sm hover:border-blue-200 hover:shadow-md">
                <div className="mx-auto mb-6 flex h-28 w-28 items-center justify-center overflow-hidden rounded-full bg-blue-50 text-blue-600">
                  {pic && isImagePath(pic) ? <img src={pic} alt="" className="h-24 w-24 rounded-full object-cover" /> : pic || <Icon size={48} aria-hidden="true" />}
                </div>
                <div className="mx-auto mb-5 flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white">{index + 1}</div>
                <h3 className="text-xl font-extrabold text-[#0B2A5B]">{step.title}</h3>
