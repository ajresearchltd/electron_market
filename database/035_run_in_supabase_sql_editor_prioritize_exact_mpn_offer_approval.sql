...
-- Gives current canonical Part Number equality priority over stale clarification state.
...
DECLARE
 ...
 v_requested_mpn text;v_offered_mpn text;
BEGIN
 ...
 SELECT roi.* INTO v_item FROM public.rfq_order_items0 AS roi WHERE roi.rfq_item_id=p_rfq_item_id AND roi.rfq_id=v_rfq.rfq_id AND roi.procurement_chain_id IS NOT DISTINCT FROM v_rfq.procurement_chain_id FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='RFQ position belongs to another procurement order.';END IF;
 SELECT sri.* INTO v_offer FROM public.supplier_response_items AS sri JOIN public.supplier_responses AS sr ON sr.id=sri.supplier_response_id
 WHERE sri.id=p_offer_item_id AND sri.is_current=true AND sri.rfq_item_id=v_item.rfq_item_id AND sri.rfq_id=v_rfq.rfq_id AND sri.procurement_chain_id=v_rfq.procurement_chain_id
   AND sr.procurement_chain_id=v_rfq.procurement_chain_id AND sr.rfq_id=v_rfq.rfq_id AND sr.supplier_id=sri.supplier_id;
 ...
 v_requested_mpn:=upper(regexp_replace(btrim(coalesce(v_item.part_number,'')),'[[:space:]._/\-]+','','g'));
 v_offered_mpn:=upper(regexp_replace(btrim(coalesce(v_offer.offered_mpn,'')),'[[:space:]._/\-]+','','g'));
 IF v_requested_mpn='' OR v_offered_mpn='' OR v_requested_mpn<>v_offered_mpn THEN
   IF NOT EXISTS(SELECT 1 FROM public.supplier_part_clarifications AS spc WHERE spc.supplier_response_item_id=v_offer.id AND spc.candidate_rfq_item_id=v_item.rfq_item_id AND spc.status='supplier_confirmed') THEN
     RAISE EXCEPTION USING ERRCODE='22023',MESSAGE='Supplier confirmation is required because the offered Part Number does not exactly match the requested Part Number.';
   END IF;
 END IF;
 ...
 WHERE psa.rfq_id=v_rfq.rfq_id AND psa.rfq_item_id=v_item.rfq_item_id AND psa.is_active AND psa.supplier_response_item_id<>v_offer.id
