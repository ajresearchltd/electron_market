...
-- Fixes RETURNS TABLE output-variable ambiguity in the human offer-approval RPC.
...
BEGIN
  ...
  UPDATE public.procurement_supplier_allocations AS psa SET
    allocated_quantity=v_quantity,selected_unit_price=v_offer.calculated_unit_price,
    currency=v_offer.currency,price_basis_quantity=v_offer.price_basis_quantity,
    price_basis_unit=v_offer.price_basis_unit,delivery_terms=coalesce(nullif(v_offer.lead_time_raw,''),'Not provided'),
    selection_reason=format('%s explicitly approved supplier offer.',initcap(v_role)),
    selected_by=p_actor_id,selected_at=now(),is_active=true,updated_at=now()
  WHERE psa.rfq_item_id=v_item.rfq_item_id AND psa.supplier_response_item_id=v_offer.id
